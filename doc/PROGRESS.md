...
| FR-031 — Physical verification (presence/location/condition assertion) | Student 1 (Jayashan) | 🟡 (`AssetVerificationScreen` — the true ad-hoc flow, `POST /api/assets/{id}/verify` — is built and has its own widget test, but **isn't wired into `app/router.dart` or navigated to from anywhere**, including its own feature's `AssetDetailActions`; grep confirms no other file references it. What's actually reachable: asset detail's "Verify" button (Officer only, `canVerifyAssetsProvider`) looks up the current user's pending verification *task* for that asset and opens `VerificationTaskDetailScreen` instead — Student 4's FR-059 flow, `PATCH /api/verification-tasks/{id}/complete` — showing "no pending task, check the task list" if none exists rather than falling through to the ad-hoc screen. So today, verifying an asset with no open campaign task isn't possible from the app at all) |
